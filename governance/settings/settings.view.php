...
<div class="wrap">
	...
	<hr/>
		<div class="combined-governance-rules">
			...
			<p class="description">Rules for a type like roles work by combining the type's governance rules with default rules. Use this tool to view rules for a specific rule type and debug permissions issues.</p>
			<select name="rule-type-selector" id="rule-type-selector" style="margin: 1rem 0 0.5rem">
				<option value="">Choose a rule type to view the rules for</option>
				<?php foreach ( $rule_types_available as $rule_type_available ) { ?>
					<?php if ( 'default' !== $rule_type_available ) { ?>
						<option value="<?php echo esc_attr( $rule_type_available ); ?>"><?php echo esc_html( $rule_type_available ); ?></option>
					<?php } ?>
				<?php } ?>
			</select>

			<select name="user-role-selector" id="user-role-selector" style="margin: 1rem 0 0.5rem" hidden>
			...
			<select name="post-type-selector" id="post-type-selector" style="margin: 1rem 0 0.5rem" hidden>
